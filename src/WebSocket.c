...
#elif defined(PAHO_ASL)
		if (net->ssl)
			ASLSocket_putdatas(net->ssl, net->socket, buf, buf_len, nulbufs);
		else
#endif
...
#elif defined(PAHO_ASL)
		if (net->ssl)
			ASLSocket_putdatas(net->ssl, net->socket, fd.wsbuf0, fd.wsbuf0len, nulbufs);
		else
#endif
...
int WebSocket_getch(networkHandles *net, char *c)
{
	...
	if (net->websocket)
	{
		...
		rc = ASLSocket_getch(net->ssl, net->socket, c);
#endif
	else
...
char *WebSocket_getdata(networkHandles *net, size_t bytes, size_t *actual_len)
{
	...
	if (net->websocket)
	{
		...
		rv = ASLSocket_getdata(net->ssl, net->socket, bytes, actual_len, &rc);
#endif
	else
...
#elif defined(PAHO_ASL)
		if (net->ssl)
			ASLSocket_putdatas(net->ssl, net->socket, fd.wsbuf0, fd.wsbuf0len /*header_len + app_data_len*/, appbuf);
		else
#endif
...
#elif defined(PAHO_ASL)
		if (net->ssl)
			rc = ASLSocket_putdatas(net->ssl, net->socket, wsdata.wsbuf0, wsdata.wsbuf0len, *bufs);
		else
#endif
...
#elif defined(PAHO_ASL)
		if (net->ssl)
			rc = ASLSocket_putdatas(net->ssl, net->socket, *buf0, *buf0len, *bufs);
		else
